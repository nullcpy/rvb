...
MSG_BODY=$(jq -rn --argjson new "$TAGS_NEW" --argjson old "$TAGS_OLD" --argjson patches "$PATCH_SOURCES_JSON" '
  [ $new | to_entries[] | . as $e
    | ($old[$e.key] // {}) as $o
    | ($e.value.beta // $e.value.prerelease // "") as $nb
    | ($o.beta // $o.prerelease // "") as $ob
...
if [ -n "$MSG_BODY" ]; then
  if [ "${TRIGGER_STABLE:-0}" = "1" ] || [ "${TRIGGER_BETA:-${TRIGGER_PRERELEASE:-0}}" = "1" ]; then
...
if [ "${EFFECTIVE_STABLE:-0}" = "1" ] || [ "${EFFECTIVE_BETA:-${EFFECTIVE_PRERELEASE:-0}}" = "1" ]; then
